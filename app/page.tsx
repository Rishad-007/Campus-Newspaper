...
export default async function Home() {
  ...
  const leadStories = stories
    .filter((story) => story.placement === "lead")
      ...
      <section className="flex flex-col gap-4 lg:flex-row lg:gap-5">
        {leadStories.length > 0 ? (
          <div className="w-full lg:w-[65%]">
            {leadStories.map((leadStory, index) => (
              <article key={leadStory.id} className="paper-surface rounded-2xl p-4 sm:p-6">
                <p className="text-xs font-semibold tracking-[0.14em] text-(--accent) uppercase">
                  {index === 0 ? "Lead Story" : "Second Lead"}
                </p>
                ...
                <h2 className="font-display mt-3 text-xl font-semibold leading-tight text-stone-900 sm:text-2xl lg:text-3xl">
                  {leadStory.title}
                </h2>
                <p
                  className={`${leadStory.locale === "bn" ? "font-bangla" : ""} mt-2 text-sm leading-6 text-stone-700 sm:text-base`}
                >
                  {leadStory.excerpt}
                </p>

                <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold tracking-wider text-stone-600 uppercase">
                ...
                <Link
                  ...
                  className="mt-4 inline-flex min-h-10 items-center justify-center rounded-full bg-(--accent) px-4 py-2 text-sm font-semibold text-white! shadow-sm transition hover:brightness-110 hover:text-white! sm:min-h-10 sm:w-auto"
                >
                  Read full report
                </Link>
              </article>
        ...
        <aside className="paper-surface w-full rounded-2xl p-4 lg:w-[35%] lg:p-6">
